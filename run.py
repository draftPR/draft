...
def start_redis(pm: ProcessManager):
    """Start Redis server."""
    try:
        # Check if already running
        result = subprocess.run(["redis-cli", "ping"], capture_output=True, timeout=1)
        if b"PONG" in result.stdout:
            print(f"{Colors.OKGREEN}{Colors.BOLD}[Redis]{Colors.ENDC} Already running")
            return
    except (FileNotFoundError, subprocess.TimeoutExpired):
        pass
    ...
    try:
        ...
        print(f"{Colors.WARNING}{Colors.BOLD}[Redis]{Colors.ENDC} Could not start: {e}")
        print(f"{Colors.WARNING}Please start Redis manually: redis-server{Colors.ENDC}")
...
def start_worker(pm: ProcessManager):
    """Start Celery worker."""
    python_path = VENV_DIR / "bin" / "python"
    celery_path = VENV_DIR / "bin" / "celery"
    
    pm.start_process(
        "Worker",
        [str(celery_path), "-A", "app.celery_app", "worker", 
