...
2. Redis pub/sub for cross-process communication (~10-50ms latency) [when TASK_BACKEND=redis]
3. Redis pipelining to minimize round-trips [when TASK_BACKEND=redis]

When TASK_BACKEND=sqlite, only in-memory broadcast is used since
worker runs in-process. Log history is read from disk files as fallback.
"""
...
class LogStreamPublisher:
    """Hybrid publisher: in-memory + optional Redis for cross-process.

    Used by workers to push logs.
    When TASK_BACKEND=sqlite, only uses in-memory broadcast (worker is in-process).
    """
    ...
    def push(
        ...
        """Push a log message via in-memory (and optionally Redis).

        In-memory broadcast is instant (<1ms).
        Redis is batched and pipelined for efficiency (only when TASK_BACKEND=redis).
        """
        msg = LogMessage(
            level=level,
            content=content,
        ...
        # 2. Queue for Redis (batched) - only when using Redis backend
...
class LogStreamSubscriber:
    ...
    When TASK_BACKEND=sqlite, only uses in-memory (worker is in-process).
    """
    ...
    def get_history(self, job_id: str) -> list[LogMessage]:
        ...
        # Fall back to Redis (only when using Redis backend)
        if not is_sqlite_backend():
            try:
                r = self._get_redis()
                history_key = _get_history_key(job_id)
    ...
    async def subscribe(self, job_id: str, max_wait_seconds: int = 1800) -> AsyncIterator[LogMessage]:
        ...
        3. Also listen to Redis pub/sub if using Redis backend
        """
        # First yield history
        ...
        try:
            ...
            if not is_sqlite_backend():
                redis_queue = asyncio.Queue(maxsize=1000)
                redis_task = asyncio.create_task(
                    self._redis_subscriber(job_id, redis_queue)
                )

            # Multiplex sources with timeout protection
