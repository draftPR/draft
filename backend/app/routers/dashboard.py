...
async def get_budget_status(
    ...
    daily_spent = await get_period_cost(db, day_start, day_start + timedelta(days=1), goal_id)
    
    # Weekly (Monday start)
    week_start = day_start - timedelta(days=day_start.weekday())
    weekly_spent = await get_period_cost(db, week_start, week_start + timedelta(weeks=1), goal_id)
    ...
    warning = (
        daily_spent >= daily_budget * 0.8 or
        weekly_spent >= weekly_budget * 0.8 or
        monthly_spent >= monthly_budget * 0.8
    )
...
async def get_sprint_metrics(db: AsyncSession, goal_id: Optional[str] = None) -> SprintMetrics:
    ...
    in_progress = sum(1 for t in tickets if t.state in [
    ...
    if done_tickets:
        cycle_times = [(t.updated_at - t.created_at).total_seconds() / 3600 for t in done_tickets]
...
async def get_agent_metrics(db: AsyncSession, goal_id: Optional[str] = None) -> AgentMetrics:
...
async def get_dashboard(
    ...
    budget = await get_budget_status(db, goal_id, daily_budget, weekly_budget, monthly_budget)
...
async def get_budget(
    ...
    return await get_budget_status(db, goal_id, daily_budget, weekly_budget, monthly_budget)
