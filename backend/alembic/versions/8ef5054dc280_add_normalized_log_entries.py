...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('entry_type', sa.Enum('THINKING', 'FILE_EDIT', 'FILE_CREATE', 'FILE_DELETE', 'COMMAND_RUN', 'TOOL_CALL', 'ERROR', 'USER_MESSAGE', 'SYSTEM_MESSAGE', 'LOADING', name='logentrytype'), nullable=False),
    ...
    op.alter_column('boards', 'updated_at',
               ...
               nullable=False,
               existing_server_default=sa.text('(CURRENT_TIMESTAMP)'))
    op.create_foreign_key(None, 'jobs', 'revisions', ['source_revision_id'], ['id'], ondelete='SET NULL')
    op.drop_column('jobs', 'agent_session_id')
    op.drop_column('jobs', 'agent_type')
    op.drop_index(op.f('ix_review_comments_revision_resolved'), table_name='review_comments')
    op.drop_index(op.f('ix_review_summaries_revision_id'), table_name='review_summaries')
    op.create_index(op.f('ix_review_summaries_revision_id'), 'review_summaries', ['revision_id'], unique=True)
    op.drop_column('tickets', 'total_cost_usd')
    op.drop_index(op.f('ix_workspaces_ticket_id'), table_name='workspaces')
    op.create_index(op.f('ix_workspaces_ticket_id'), 'workspaces', ['ticket_id'], unique=True)
...
def downgrade() -> None:
    ...
    op.drop_index('ix_normalized_log_entries_entry_type', table_name='normalized_log_entries')
    ...
    op.drop_index(op.f('ix_review_summaries_revision_id'), table_name='review_summaries')
    op.create_index(op.f('ix_review_summaries_revision_id'), 'review_summaries', ['revision_id'], unique=False)
    op.create_index(op.f('ix_review_comments_revision_resolved'), 'review_comments', ['revision_id', 'resolved'], unique=False)
    op.add_column('jobs', sa.Column('agent_type', sa.VARCHAR(length=50), nullable=True))
    op.add_column('jobs', sa.Column('agent_session_id', sa.VARCHAR(length=36), nullable=True))
