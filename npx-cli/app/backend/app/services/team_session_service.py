...
class TeamSessionService:
    ...
    # ------------------------------------------------------------------
    # Planning-specific team launch
    # ------------------------------------------------------------------

    def launch_planning_team(
        self,
        board_id: str,
        goal_title: str,
        ...
        api_base_url: str = "http://localhost:8000",
        ticket_json_schema: str = "",
    ) -> tuple[list[TeamAgentSession], str]:
        """Launch a planning team to generate tickets for a goal.

        Unlike launch_team(), this:
        - Runs in repo_root (no worktree isolation needed — read-only)
        - Uses planning-specific prompts
        - Always enables YOLO (read-only analysis)
        - Uses a synthetic ticket_id for message board scoping
        ...
        if not tmux_manager.is_tmux_available():
            raise RuntimeError(
                "tmux is required for multi-agent planning but is not installed."
            )
        ...
        # Synthetic ticket_id to scope board messages for this planning session
        synthetic_ticket_id = f"planning-{uuid4().hex[:12]}"

        # Inject board CLI into repo root
        ...
        try:
            # Use a dummy job_id for DB records
            dummy_job_id = f"planning-{uuid4().hex[:12]}"
            ...
            launched_tmux_names.append(orch_session.tmux_session_name)
            self.board_service.subscribe(
                board_id, synthetic_ticket_id, orch_session.session_uuid
            )

            # Orchestrator planning prompt
            orch_prompt = self._build_planning_orchestrator_prompt(
            ...
            tmux_manager.send_text(orch_session.tmux_session_name, orch_prompt)
            ...
            for worker_member in workers:
                ...
                launched_tmux_names.append(worker_session.tmux_session_name)
                self.board_service.subscribe(
                    board_id, synthetic_ticket_id, worker_session.session_uuid
                )

                worker_prompt = self._build_planning_worker_prompt(
                    goal_title=goal_title,
                    goal_description=goal_description,
                    member=worker_member,
                    roster_str=roster_str,
            ...
            self.db.commit()
            logger.info(
                "Launched planning team of %d agents for goal '%s'",
                len(sessions),
                goal_title,
            )
            ...
            logger.error(
                "Planning team launch failed after %d/%d agents: %s",
                len(launched_tmux_names),
                len(team.members),
                exc,
            )
    ...
    def _build_planning_orchestrator_prompt(
        self,
        goal_title: str,
        goal_description: str | None,
        roster_str: str,
        ...
        """Build prompt for the Team Lead in a planning session."""
        desc = goal_description or "No additional description."

        return (
            "You are the Team Lead orchestrating a PLANNING session.\n"
            "Your goal is to generate implementation tickets — NOT to write code.\n\n"
            f"# Goal: {goal_title}\n\n"
            f"## Description\n{desc}\n\n"
            f"{existing_tickets_str}"
            f"## Your Team\n{roster_str}\n\n"
            "## Your Task\n"
            "1. Ask the @PM to analyze the goal and break it into requirements\n"
            "2. Ask the @CodeExplorer to explore the codebase and find relevant files/patterns\n"
            "3. Wait for their findings (check the board with: .draft/board-cli.sh read)\n"
            "4. Synthesize their input into implementation tickets\n"
            "5. When done, post DONE: followed by a JSON block with the tickets\n\n"
            f"## Output Format\n{ticket_json_schema}\n\n"
            "## Communication\n"
            'Use: .draft/board-cli.sh post "your message"\n'
            "Read: .draft/board-cli.sh read\n"
            "Check: .draft/board-cli.sh check\n\n"
            "## Completion\n"
            "When you have synthesized all findings into tickets, post:\n"
            '.draft/board-cli.sh post "DONE: ```json\n{"tickets": [...]}\n```"\n\n'
            "The JSON must follow the output format above exactly.\n"
            "Begin by delegating to your team."
        )

    def _build_planning_worker_prompt(
        self,
        goal_title: str,
        goal_description: str | None,
        member: AgentTeamMember,
        ...
        """Build prompt for a worker agent in a planning session."""
        desc = goal_description or "No additional description."
        behavior = member.behavior_prompt or f"You are {member.display_name}."
        ...
        if member.role == "pm":
            role_instructions = (
                ...
                "5. Post your full analysis to the board for the Team Lead\n\n"
                "Do NOT wait for assignment — start analyzing immediately.\n"
            )
            ...
            role_instructions = (
                ...
                "5. Post your full analysis to the board for the Team Lead\n\n"
                "Do NOT wait for assignment — start exploring immediately.\n"
            ...
            role_instructions = (
                ...
                "Post your findings to the board for the Team Lead.\n"
                "Do NOT wait for assignment — start analyzing immediately.\n"
            )

        return (
            f"{behavior}\n\n"
            f"# Planning Session — Goal: {goal_title}\n\n"
            f"## Description\n{desc}\n\n"
            f"{role_instructions}"
            f"## Your Team\n{roster_str}\n\n"
            "## Communication\n"
            ...
            "Check: .draft/board-cli.sh check\n\n"
            "## Status Reporting\n"
            "  ||PULSE:STATUS Analyzing goal||\n"
            "  ||PULSE:SUMMARY <one sentence>||\n\n"
